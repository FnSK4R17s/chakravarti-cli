...
impl HistoryService {
    ...
    pub fn load_history(&self, spec_name: &str) -> Result<RunHistory> {
        ...
        match serde_yaml::from_str::<RunHistory>(&content) {
            ...
            Err(e) => {
                eprintln!(
                    "[HistoryService] Warning: Failed to parse history file, returning empty: {}",
                    e
                );
    ...
    pub fn save_history(&self, history: &RunHistory) -> Result<()> {
        ...
        // Serialize to YAML
        let content =
            serde_yaml::to_string(history).context("Failed to serialize history to YAML")?;
        ...
        fs::rename(&temp_path, &path).context("Failed to rename temp file to history file")?;
