...
pub async fn get_spec(
    ...
    match std::fs::read_to_string(&spec_path) {
        Ok(content) => match serde_yaml::from_str::<SpecDetail>(&content) {
...
pub async fn save_spec(Json(payload): Json<SaveSpecPayload>) -> impl IntoResponse {
    let cwd = std::env::current_dir().unwrap_or_else(|_| PathBuf::from("."));
    let spec_path = cwd.join(".specs").join(&payload.name).join("spec.yaml");

    match serde_yaml::to_string(&payload.spec) {
        Ok(yaml) => match std::fs::write(&spec_path, yaml) {
...
pub async fn create_spec(Json(payload): Json<CreateSpecPayload>) -> impl IntoResponse {
...
pub async fn get_clarifications(
    ...
    if let Ok(content) = std::fs::read_to_string(&spec_path) {
        ...
        if let Ok(spec) = serde_yaml::from_str::<SpecWithClarifications>(&content) {
            let clarifications: Vec<Clarification> = spec
                .clarifications
                .into_iter()
                .map(|c| Clarification {
                    topic: c.topic,
...
pub async fn submit_clarifications(
    ...
    if let Ok(content) = std::fs::read_to_string(&spec_path) {
        if let Ok(mut spec_value) = serde_yaml::from_str::<serde_yaml::Value>(&content) {
            ...
            if let Some(clarifications) = spec_value
                ...
                .and_then(|c| c.as_sequence_mut())
            {
                    ...
                    for clarif in clarifications.iter_mut() {
                        if let Some(topic) = clarif.get("topic").and_then(|t| t.as_str()) {
                            if topic == answer.topic {
                                clarif["resolved"] =
                                    serde_yaml::Value::String(answer.answer.clone());
                                resolved_count += 1;
                            }
                        }
                    }
                ...
                // Count remaining
                let remaining = clarifications
                    .iter()
                    .filter(|c| {
                        c.get("resolved").is_none()
                            || c.get("resolved").and_then(|r| r.as_str()).is_none()
                    })
                    .count();
