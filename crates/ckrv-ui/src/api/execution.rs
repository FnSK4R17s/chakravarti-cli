...
use crate::models::log::{
    LogDeleteResponse, LogHistoryRequest, LogHistoryResponse, LogTailResponse,
};
...
static EXECUTIONS: Lazy<Mutex<HashMap<String, ExecutionState>>> =
...
pub async fn start_execution(
    ...
    let handle = tokio::spawn(async move {
        ...
        if let Err(e) = engine
            .run_spec(spec_name, dry_run, executor_model, agent, resume_run_id)
            .await
        {
    ...
    println!(
        "Execution initialized: {} for spec {}",
        payload.run_id, payload.spec
    );
    ...
    Json(StartExecutionResponse {
        ...
        run_id: payload.run_id,
        message: Some(
            "Execution started in background. Connect WebSocket to stream logs.".to_string(),
        ),
    })
}
...
pub async fn stop_execution(Json(payload): Json<StopExecutionRequest>) -> impl IntoResponse {
    let mut executions = EXECUTIONS.lock().unwrap();
    ...
    if let Some(state) = executions.get_mut(&payload.run_id) {
        if let Some(handle) = state.abort_handle.take() {
            ...
            let _ = state
                .log_tx
                .send(LogMessage::new("error", "Execution aborted by user."));
...
pub async fn get_execution_logs_tail(
    ...
    match log_store.read_tail(&execution_id, count) {
        Ok((logs, total_count)) => Json(LogTailResponse {
            execution_id,
            logs,
            total_count,
        }),
...
pub async fn delete_execution_logs(
    ...
    match log_store.delete(&execution_id) {
        Ok(deleted_lines) => Json(LogDeleteResponse {
            success: true,
            execution_id,
            deleted_lines,
        }),
