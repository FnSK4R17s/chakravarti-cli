...
const COMMON_OPTIONS: &[CommonOption] = &[
    ...
    CommonOption {
        label: "Auto mode",
        action: OptionAction::Flag("--yes --approval-mode=auto"),
        description: "Run autonomously without approval prompts",
        agents: &[AgentType::Qwen],
    },
];
...
#[command(
    long_about = "Spawn an interactive AI agent terminal session.\n\n\
                  Quickly launch any configured agent (Claude, OpenRouter, Z.AI, Codex, Kilo Code, Qwen Code) \
...
fn build_agent_command(agent: &AgentConfig) -> anyhow::Result<(String, Vec<(String, String)>)> {
    ...
    match agent.agent_type {
        ...
        AgentType::Qwen => {
            // Qwen Code uses ~/.qwen/ for credentials - no extra env vars needed
        }
    }
