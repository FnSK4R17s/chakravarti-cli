...
const COMMON_OPTIONS: &[CommonOption] = &[
    ...
    CommonOption {
        label: "Auto mode",
        action: OptionAction::Flag("run --auto"),
        description: "Run Factory Droid in autonomous mode",
        agents: &[AgentType::FactoryDroid],
    },
];
...
#[command(
    long_about = "Spawn an interactive AI agent terminal session.\n\n\
                  Quickly launch any configured agent (Claude, OpenRouter, Z.AI, Codex, Kilo Code, Factory Droid) \
    ...
    if json {
        ...
        for agent in &enabled_agents {
            let type_badge = match agent.agent_type {
                ...
                AgentType::FactoryDroid => "factory",
            };
...
fn build_agent_command(agent: &AgentConfig) -> anyhow::Result<(String, Vec<(String, String)>)> {
    ...
    match agent.agent_type {
        ...
        AgentType::FactoryDroid => {
            // Factory Droid uses file-based auth (~/.factory/) - no extra env vars needed
        }
    }
