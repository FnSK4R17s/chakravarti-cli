...
pub async fn execute(args: QaArgs, json: bool, ui: &UiContext) -> anyhow::Result<()> {
    match args.command {
        ...
        QaSubcommand::Report { base, full, output } => execute_report(&base, full, output, json, ui).await,
    }
}

/// Execute QA review
async fn execute_review(base: &str, output: Option<String>, json: bool, ui: &UiContext) -> anyhow::Result<()> {
    if !json {
        println!("{}", Banner::new("CKRV QA").subtitle("Code Review").render(&ui.theme));
    }
    ...
    if json {
        ...
        println!("   The QA agent ({}) would provide more detailed analysis.", agent.name);
    }
...
async fn execute_bugs(base: &str, json: bool, ui: &UiContext) -> anyhow::Result<()> {
    if !json {
        println!("{}", Banner::new("CKRV QA").subtitle("Bug Analysis").render(&ui.theme));
    }
    ...
    if !json {
        println!("🤖 Using agent: {} ({})\n", agent.name, agent.id);
        println!("🔍 Scanning for potential bugs in changes vs '{}'...\n", base);
    }
    ...
    let bug_issues: Vec<_> = all_issues
        .into_iter()
        .filter(|i| matches!(i.category, IssueCategory::PotentialBug | IssueCategory::ErrorHandling))
        .collect();
...
async fn execute_report(base: &str, full: bool, output: Option<String>, json: bool, ui: &UiContext) -> anyhow::Result<()> {
    if !json {
        println!("{}", Banner::new("CKRV QA").subtitle("Full Report").render(&ui.theme));
    }
    ...
    if !json {
        println!("🤖 Using agent: {} ({})\n", agent.name, agent.id);
        println!("📊 Generating {} QA report for changes vs '{}'...\n", 
            if full { "full" } else { "standard" }, base);
    }
    ...
    if json {
        ...
        report.push_str(&report_generator::generate_qa_report(&issues, &branch, base));
    ...
    for file in changed_files {
        ...
        if ext == "rs" && file.lines_added > 50 {
            issues.push(QAIssue {
                ...
                suggestion: Some("Add /// doc comments to public functions and structs".to_string()),
            });
            issue_id += 1;
        }
    }
