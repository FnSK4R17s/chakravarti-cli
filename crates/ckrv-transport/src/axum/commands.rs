...
//! CLI operations (which call `std::process::Command`) off the async runtime.
...
async fn run_init(State(state): State<AppState>) -> impl IntoResponse {
    match tokio::task::spawn_blocking(move || run_init_handler(&state)).await {
...
async fn run_git_init(State(state): State<AppState>) -> impl IntoResponse {
    match tokio::task::spawn_blocking(move || run_git_init_handler(&state)).await {
        Ok(Ok(result)) => Json(result).into_response(),
        Ok(Err(e)) => e.into_response(),
        Err(e) => TransportError::Internal(format!("Task panicked: {e}")).into_response(),
    }
...
async fn run_spec_tasks(State(state): State<AppState>) -> impl IntoResponse {
    match tokio::task::spawn_blocking(move || run_spec_tasks_handler(&state)).await {
        Ok(Ok(result)) => Json(result).into_response(),
        Ok(Err(e)) => e.into_response(),
        Err(e) => TransportError::Internal(format!("Task panicked: {e}")).into_response(),
    }
...
async fn run_execute(State(state): State<AppState>) -> impl IntoResponse {
    match tokio::task::spawn_blocking(move || run_execute_handler(&state)).await {
        Ok(Ok(result)) => Json(result).into_response(),
        Ok(Err(e)) => e.into_response(),
        Err(e) => TransportError::Internal(format!("Task panicked: {e}")).into_response(),
    }
...
async fn run_diff(
    ...
    match tokio::task::spawn_blocking(move || run_diff_handler(&state, request)).await {
        Ok(Ok(result)) => Json(result).into_response(),
        Ok(Err(e)) => e.into_response(),
        Err(e) => TransportError::Internal(format!("Task panicked: {e}")).into_response(),
    }
...
async fn run_verify(
    ...
    match tokio::task::spawn_blocking(move || run_verify_handler(&state, request)).await {
        Ok(Ok(result)) => Json(result).into_response(),
        Ok(Err(e)) => e.into_response(),
        Err(e) => TransportError::Internal(format!("Task panicked: {e}")).into_response(),
    }
...
async fn run_promote(
    ...
    match tokio::task::spawn_blocking(move || run_promote_handler(&state, request)).await {
        Ok(Ok(result)) => Json(result).into_response(),
        Ok(Err(e)) => e.into_response(),
        Err(e) => TransportError::Internal(format!("Task panicked: {e}")).into_response(),
    }
...
async fn run_fix(
    ...
    match tokio::task::spawn_blocking(move || run_fix_handler(&state, request)).await {
        Ok(Ok(result)) => Json(result).into_response(),
        Ok(Err(e)) => e.into_response(),
        Err(e) => TransportError::Internal(format!("Task panicked: {e}")).into_response(),
    }
